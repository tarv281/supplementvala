           ...
           // $isb_price['id'] = get_the_ID();
            ...
            if(!empty($isb_curr_set)){
                ...
                switch ($curr_badge[0]['style']){
                    case  'isb_style_cloud': $isb_price['type'] == 'simple'?$data['sale_percentage'] = $discount  . ' ' . 'save' . ' ' . $difference_display: $data['sale_percentage'] = $discount  . ' ' . 'Up to' . ' ' . $difference_display;
                                             break;

                    case 'isb_style_inline': $isb_price['type'] == 'simple'?$data['sale_percentage'] = $discount  . ' ' . 'save' . ' ' . $difference_display: $data['sale_percentage'] = $discount  . ' ' . 'Up to' . ' ' . $difference_display;
                                              break;
                    default               :$data['sale_percentage']=$discount;
                }
