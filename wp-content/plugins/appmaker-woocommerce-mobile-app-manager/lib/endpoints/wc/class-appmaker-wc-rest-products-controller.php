   ...
   /*     $images            = $this->get_images( $variation, true, APPMAKER_WC_Helper::get_id( $product ) );
    ...
    public function get_sale_percentage( $product ) {
        ...
        if ( ! is_a( $product, 'WC_Product_Variable' ) ) {
            $maximumper = ( $product->is_on_sale() && 0 != $product->get_regular_price() && ( $product->get_regular_price() > $product->get_sale_price() ) ) ? round( ( ( $product->get_regular_price() - $product->get_sale_price() ) / $product->get_regular_price() ) * 100 ) : 0;
        } else {
    ...
    public function get_items( $request ) {
        ...
        $query_result = apply_filters('appmaker_wc_product_query_result',$query_result);
