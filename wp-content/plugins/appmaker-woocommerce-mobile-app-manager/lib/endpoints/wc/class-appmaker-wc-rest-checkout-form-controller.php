...
class APPMAKER_WC_REST_Checkout_Form_Controller extends APPMAKER_WC_REST_Controller {
    ...
    public function register_routes() {
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/form/(?P<field>[\a-z]+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/shipping_methods', array(
            array(
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/payment_gateways', array(
            array(
                'methods'             => WP_REST_Server::CREATABLE,
    ...
    private function getMethodsInArray( $methods ) {
        ...
        foreach ( $methods as $method ) {
            $return[] = array(
                ...
                'cost'         => $method->cost,
                'cost_display' => APPMAKER_WC_Helper::get_display_price( $method->cost ),
                'taxes'        => $method->taxes,
    ...
    public function payment_gateways( $request ) {
        ...
        foreach ( $available_payment_gateways as $key => $gateway ) {
            ...
            if ( apply_filters( 'appmaker_wc_payment_gateway_' . $gateway->id . '_enabled', true ) ) {
                $return['gateways'][] = apply_filters( 'appmaker_wc_payment_gateway_' . $gateway->id, array(
                    'id'                => $gateway->id,
       ...
       foreach($return['gateways'] as $gateways=>$gateway){
        ...
        if($return['skip_payment_gateways']!==0) {
            ...
            $billing_array = array('first_name' => $request['billing_first_name'],
            ...
            if(isset($request['shipping_first_name'])) {
                $shipping_array = array('first_name' => $request['shipping_first_name'],
                ...
                $shipping_array  = array('first_name' => $request['billing_first_name'],
                    'last_name' => $request['billing_last_name'],
                    'company' => $request['billing_company'],
                    'address_1' => $request['billing_address_1'],
                    'address_2' => $request['billing_address_2'],
    ...
    public function prepare_state_response( $countries, $states_c ) {
        ...
        foreach ( $countries as $key => $country ) {
            $return[ $key ] = array( 'name' => html_entity_decode( $country ), 'states' => array() );
            if ( isset( $states_c[ $key ] ) && is_array( $states_c[ $key ] ) ) {
                foreach ( $states_c[ $key ] as $state_key => $state ) {
                    $return[ $key ]['states'][] = array( 'id' => $key, 'name' => html_entity_decode( $state ) );
                }
            }
        }
